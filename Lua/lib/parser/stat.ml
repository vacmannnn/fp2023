...
let%expect_test "parse_while" =
  pp pp_block parse_block "while 1+2 do x = 1 b = 2 end" ;
  [%expect
    {|
    [(Stat_while (Exp_true,
        [(Stat_assign (Nonlocal, (Lhs_ident "x"), (Exp_number 1.)));
...
(* let%expect_test "parse fun" = pp pp_block parse_block "if true then x = 2 end" *)
...
(* let%expect_test "parse if5" = pp pp_block parse_block "if a <= 2 then a=2 end" *)
