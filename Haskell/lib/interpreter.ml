...
module Environment (M: MONAD_ERROR) = struct
  type environment = (string, (value, err) M.t) Hashtbl.t
  ...
  let pp_environment fmt env =
    ...
    Hashtbl.iter (fun key value ->
      bindings := (key, value) :: !bindings) env;
    let pp_binding fmt (key, value) =
      Format.fprintf fmt "%s -> %a" key pp_value value
    in
    Format.fprintf fmt "{ ";
    Format.pp_print_list ~pp_sep:(fun fmt () -> Format.fprintf fmt "; ") pp_binding fmt !bindings;
    Format.fprintf fmt " }"
  ...
  let pp_err fmt = function
    ...
    | EvalError (val1, val2) -> fprintf fmt "EvalError: %a # %a" pp_value val1 pp_value val2
  ;;
...
module Eval (M : MONAD_ERROR) : sig
  ...
  let rec eval (env: environment) (expr: expr): (value, err) t =  
    match expr with
    | ExprBinOp (op, e1, e2) -> 
    ...
    | ExprIf (cond, then_expr, else_expr) -> thunk (fun () ->
    ...
    and match_pattern = fun env pat value ->
      match pat with
      | PatVar x -> Hashtbl.replace env x value; return env
      (* | PInt n ->
        (match value with
...
let test_expr = (DeclLet
((PatVar "yx"),
 (ExprIf ((ExprLit (LitBool false)), (ExprLit (LitInt 45)),
